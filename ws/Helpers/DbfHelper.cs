	...
	public class DbfHelper
	{
		...
		static public void readDbf()
		{
			...
			foreach (DataRow dr in dbfTable.Rows)
			{
				...
				{
					var s = GetDbValue<string>(string.Format("Select Id From {0} Where isware=0 and N='{1}'", Common.SProdTable, group));
					if (string.IsNullOrEmpty(s))
					{
						cmd = string.Format("Insert {0} (Id,IdP,N,isware) values({1},0,'{2}',0)",
								Common.SProdTable, iGrpId, group);
						var ret = BaseRepository.ExecuteCommand(cmd);
				...
				cmd = string.Format("Select * From {0} Where Id='{1}'", Common.SProdTable, dr["code"]);
				using (var reader = BaseRepository.ExecuteReaderEx("BAsket", cmd, null))
				{
					...
					else
					{
						...
						cmd = string.Format(
								...
								Common.SProdTable, dr["code"], group, dr["name"],
		...
		{
			var iGrpId = GetDbValue<int>(string.Format("select min(cast(Id as int)) as Id from {0} where isware=0 and left(Id, 1)<>'.'", Common.SProdTable));
			iGrpId--;
			return iGrpId;
			/*            using (var reader = BaseRepository.ExecuteReaderEx("BAsket", cmd, null))
						{
				...
				while (i < xarray.Count())
				{
					...
					do
					{
						...
						if (fiName == "group")
						{
							...
							{
								cmd = string.Format("select r_war from {0} where isware=0 and name='{1}'", Common.SProdTable, val);
								using (var reader = BaseRepository.ExecuteReaderEx("BAsket", cmd, null))
								{
									if (reader.Read())
									{
					...
					cmd = string.Format("select * from {0} where r_war='{1}'", Common.SProdTable, warFields["r_war"]);
						...
						else
						{
							...
							cmd =
								string.Format(
									...
									" values('{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}', 1)",
									Common.SProdTable, warFields["r_war"], warFields["r_hwar"], warFields["name"],
